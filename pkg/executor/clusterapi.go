...
func clusterAPISteps(path string) []*Step {
	...
	return []*Step{
		...
		},
		{
			...
			Target:  pluralFile(path, "ONCE"),
			Command: "plural",
			Args:    []string{"--bootstrap", "wkspace", "helm", sanitizedPath, "--skip", "cluster-api-cluster", "--set", "bootstrap-operator.operator.bootstrapMode=true", "--set", "bootstrap-operator.operator.moveCluster=true"},
			Sha:     "",
			Retries: 2,
		},
		{
		...
		},
		{
			...
			Target:  pluralFile(path, "ONCE"),
			Command: "plural",
			Args:    []string{"--bootstrap", "wkspace", "helm", sanitizedPath, "--set", "bootstrap-operator.operator.bootstrapMode=true", "--set", "bootstrap-operator.operator.moveCluster=true"},
			Sha:     "",
			Retries: 5,
		},
		{
