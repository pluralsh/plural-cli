...
func (f *Flash) Warn(msg string) {
	// log.Warn().Msg(msg) //TODO: re-add logging
...
func (f *Flash) Err(err error) {
	// log.Error().Msg(err.Error()) //TODO: re-add logging
...
func (f *Flash) Errf(fmat string, args ...interface{}) {
	...
	fmt.Sprintf("%s", err) //TODO: remove
	// log.Error().Err(err).Msgf(fmat, args...) //TODO: re-add logging
