...
func (m Model) View(width, height int) string {
	...
	if m.mode == modeDetail && m.detail.Name != "" {
		title = "Notifications · " + m.detail.Name
	}
...
func (m Model) headerStatus() string {
	...
	switch m.mode {
	case modeDetail:
		return m.theme.Success.Render(loCoalesce(m.detail.Type, "sink"))
	case modeList:
		if m.filter != "" {
			return m.theme.Muted.Render(fmt.Sprintf("%d matching", len(m.page.Items)))
		}
		return m.theme.Success.Render(fmt.Sprintf("%d sinks", len(m.page.Items)))
	default:
		return m.theme.Muted.Render("notifications")
	}
}
...
func (m Model) bodyAndHelp(width int) (string, string) {
	if m.mode == modeFilter {
		lines := []string{
			m.theme.Muted.Render("Filter by name, type, url, or id."),
			"",
			m.filterInput.View(),
		}
		return page.Panel(m.theme, "Filter sinks", lines, width, 6, true), "enter apply · esc cancel"
	}
	if m.needsAuth {
		lines := []string{
			m.theme.Warning.Render("○ Console is not connected"),
			m.theme.Muted.Render("  Connect a Console profile to browse notification sinks."),
			"",
...
func (m Model) listTitle() string {
	if m.filter != "" {
		return "Sinks · filter “" + m.filter + "”"
	}
	return "Notification sinks"
}

func (m Model) listLines(width int) []string {
	if m.loading && len(m.page.Items) == 0 {
		return []string{m.theme.Warning.Render("◌ Loading notification sinks…")}
	}
	if m.err != nil {
		return []string{
			m.theme.Danger.Render("✗ Unable to load notification sinks"),
	...
	if len(m.page.Items) == 0 {
		return []string{
			m.theme.Warning.Render("○ No notification sinks found"),
			m.theme.Muted.Render("  Adjust the filter or connect another Console."),
		}
	}
	nameWidth := max(12, min(24, width/4))
	typeWidth := 8
	urlWidth := max(20, min(40, width/2))
	lines := []string{m.theme.Muted.Render("  " + pad("NAME", nameWidth) + " " + pad("TYPE", typeWidth) + " URL")}
	...
	for i := start; i < end; i++ {
		...
		url := loCoalesce(item.URL, "—")
		row := cursor + pad(item.Name, nameWidth) + " " + pad(item.Type, typeWidth) + " " + pad(url, urlWidth)
...
func (m Model) detailLines() []string {
	if m.loading {
		return []string{m.theme.Warning.Render("◌ Loading sink detail…")}
	}
	if m.err != nil {
		return []string{m.theme.Danger.Render("✗ Unable to load sink"), m.theme.Danger.Render(m.err.Error())}
	}
	lines := []string{
		m.labelValue("Name", m.detail.Name),
		m.labelValue("Type", loCoalesce(m.detail.Type, "—")),
		m.labelValue("URL", loCoalesce(m.detail.URL, "—")),
		m.labelValue("ID", m.detail.ID),
	}
	if len(m.detail.Bindings) > 0 {
		lines = append(lines, "", m.theme.Muted.Render("Bindings"))
		for _, binding := range m.detail.Bindings {
			lines = append(lines, "  "+binding.Kind+"  "+binding.Name)
		}
	}
	return lines
}
