// Package notifications implements the read-only Console notification sinks browser.
...
// Model owns Notifications-screen interaction state.
type Model struct {
	ctx       context.Context
	loader    notificationsbridge.Loader
	theme     theme.Theme
	mode      mode
	loading   bool
	err       error
...
func New(ctx context.Context, loader notificationsbridge.Loader, t theme.Theme) Model {
	input := textinput.New()
	input.Prompt = "› "
	input.Placeholder = "filter notification sinks"
	input.CharLimit = 128
...
func (m Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case initMsg:
		m.mode = modeList
		m.page = notificationsbridge.Page{}
		m.cursor = 0
		m.after = nil
		m.prevCursors = nil
		m.err = nil
