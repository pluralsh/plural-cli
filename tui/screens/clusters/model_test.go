...
type fakeLoader struct {
	items  []clustersbridge.Summary
	detail clustersbridge.Detail
	err    error
}

func (f *fakeLoader) List(context.Context, string) ([]clustersbridge.Summary, error) {
...
func TestOpenClusterDetailAndBack(t *testing.T) {
	...
	if model.mode != modeList || len(model.items) != 2 {
		t.Fatalf("list state = mode=%d count=%d", model.mode, len(model.items))
	}
...
func TestClustersGoldens(t *testing.T) {
	...
	list.loading = false
	list.mode = modeList
	list.items = []clustersbridge.Summary{
		{ID: "c1", Name: "production", Handle: "prod-eu", Version: "1.30.2", Distro: "EKS"},
		{ID: "c2", Name: "staging", Handle: "staging", Version: "1.29.0", Distro: "EKS"},
