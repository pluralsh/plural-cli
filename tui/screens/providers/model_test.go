...
type fakeLoader struct {
	page   providersbridge.Page
	detail providersbridge.Detail
	err    error
}

func (f *fakeLoader) List(context.Context, *string, string) (providersbridge.Page, error) {
	return f.page, f.err
}
func (f *fakeLoader) Get(context.Context, string) (providersbridge.Detail, error) {
...
func TestOpenProviderDetailAndBack(t *testing.T) {
	loader := &fakeLoader{
		page: providersbridge.Page{Items: []providersbridge.Summary{
			{ID: "pr1", Name: "aws-west", Cloud: "aws", Editable: "true", RepoURL: "https://github.com/acme/infra"},
			{ID: "pr2", Name: "gcp-east", Cloud: "gcp"},
		}},
		detail: providersbridge.Detail{
			Summary: providersbridge.Summary{ID: "pr1", Name: "aws-west", Cloud: "aws", Namespace: "infra", Editable: "true", RepoURL: "https://github.com/acme/infra"},
			Service: "infra/provider",
			Credentials: []providersbridge.Credential{
	...
	if !strings.Contains(model.View(80, 24), "aws-west") {
		t.Fatalf("list missing name:\n%s", model.View(80, 24))
	}

	model, cmd := model.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	model, _ = model.Update(cmd())
	if model.mode != modeDetail || model.detail.Name != "aws-west" {
		t.Fatalf("detail = %#v mode=%d", model.detail, model.mode)
	}
	if !strings.Contains(model.View(80, 24), "aws-creds") {
		t.Fatalf("detail missing credential:\n%s", model.View(80, 24))
	}
...
func TestNextPrevPage(t *testing.T) {
	loader := &fakeLoader{
		page: providersbridge.Page{
			Items:     []providersbridge.Summary{{ID: "pr1", Name: "a", Cloud: "aws"}},
			EndCursor: "pr1",
			HasNext:   true,
		},
	}
	...
	loader.page = providersbridge.Page{Items: []providersbridge.Summary{{ID: "pr2", Name: "b", Cloud: "gcp"}}}
	model, _ = model.Update(cmd())
	if model.after == nil || *model.after != "pr1" || len(model.prevCursors) != 1 {
...
func TestProvidersGoldens(t *testing.T) {
	list := New(t.Context(), nil, theme.New(colorprofile.ASCII))
	list.loading = false
	list.mode = modeList
	list.page = providersbridge.Page{Items: []providersbridge.Summary{
		{ID: "pr1", Name: "aws-west", Cloud: "aws", Editable: "true", RepoURL: "https://github.com/acme/infra"},
		{ID: "pr2", Name: "gcp-east", Cloud: "gcp", Editable: "false"},
		{ID: "pr3", Name: "azure-central", Cloud: "azure", RepoURL: "https://github.com/acme/azure"},
	}, HasNext: true, EndCursor: "pr3"}

	detail := list
	detail.mode = modeDetail
	detail.detail = providersbridge.Detail{
		Summary: providersbridge.Summary{ID: "pr1", Name: "aws-west", Cloud: "aws", Namespace: "infra", Editable: "true", RepoURL: "https://github.com/acme/infra"},
		Service: "infra/provider",
		Credentials: []providersbridge.Credential{
			{Name: "aws-creds", Namespace: "infra", Kind: "Secret"},
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			golden := filepath.Join("testdata", "providers-"+tc.name+".golden")
...
func TestWriteProvidersGoldens(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got := normalizeView(tc.model.View(tc.width, tc.height)) + "\n"
		if err := os.WriteFile(filepath.Join("testdata", "providers-"+tc.name+".golden"), []byte(got), 0o644); err != nil {
			t.Fatal(err)
		}
	}
}
